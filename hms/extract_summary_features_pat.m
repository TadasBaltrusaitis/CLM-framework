...
%     feats_all = cat(2, feats_pat_mean, feats_doc_mean, feats_pat_std, feats_doc_std);
    feats_all = cat(2, feats_pat_mean, feats_pat_std);
...
for scale=1:size(scales,2)
    ...
    sc = scales(:,scale);
    [c_scale,ps] = corr(features, sc);
    
    c_scale_sq = c_scale .^2;
    
    [best_c, ind_best] = max(c_scale_sq);
    
    fprintf('%s, best %s, corr - %.3f, p - %.3f\n', scale_names{scale}, feature_names_all{ind_best}, c_scale(ind_best), ps(ind_best));
...
for scale=1:size(scales,2)
    ...
    [c_scale, p] = corr(preds_all(:,scale), scales(:,scale));
    
    fprintf('%s, corr - %.3f, p-%.3f\n', scale_names{scale}, c_scale, p);

function [ a, R, T, error, shapeOrtho ] = fit_PDM_ortho_janus( M, shape2D, init_rot)
...
%   Detailed explanation goes here
    ...
    % pick the more robust measure to normalise (width or height)
    if(width_model > height_model)
        a = (bounding_box(3) - bounding_box(1)) / width_model;
    else
        a = (bounding_box(4) - bounding_box(2))/ height_model;
    end
    m = m * a;
    
    tx = mean(shape2D(:,1)) + mean(m(1,:));
    ty = mean(shape2D(:,2)) - mean(m(2,:));    
    
    R = Euler2Rot(init_rot); 
    T = [tx; ty];
    
    currShape = getShapeOrtho(M, R, T, a);
    
    error = getRMSerror(currShape, shape2D);
    p_global = [a; init_rot'; T];
        
    if(hidden)
        shapeOrtho = getShapeOrtho(M_old, R, T, a);
    else
        shapeOrtho = currShape;
    end    
end

function [shape2D] = getShapeOrtho(M, R, T, a)
...
function J = CalcRigidJacobian(M, p_global)
...
function [rigid] = CalcReferenceUpdate(params_delta, current_global)

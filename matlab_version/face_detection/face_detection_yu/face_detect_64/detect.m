...
% matlabpool open 4
% tic;
% parfor ss = (model.interval+1):length(pyra.feat)
%     resp{ss} = fconv(pyra.feat{ss}, filters, 1, length(filters));
% end
% toc;
% ress_ = fconv2(pyra.feat,filters,model.interval+1, length(pyra.feat), 1, length(filters));
    ...
    for rlevel = levels(randperm(length(levels))),
       ...
       for k = 1:numparts,
            ...
            if isempty(resp{level}),
                resp{level} = fconvMT(pyra.feat{level},filters,1,length(filters)); %1,length(filters));
            end
            parts(k).score = resp{level}{f};
%             parts(k).score = ress_{rlevel-model.interval,f};
            parts(k).level = level;

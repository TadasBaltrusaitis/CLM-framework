function CLNF_meta_dataset_on_validation()
...
for i=1:numel(images)
    ...
    if(multi_view)
        ...
        views = [0,0,0; 0,-45,0; 0,45,0; 20,0,0; 20,0,0;];
        views = views * pi/180;                                                                                     
        ...
        % Correct the widths
        bbox(3) = bbox(:,3) * 0.7519;
        bbox(4) = bbox(:,4) * 0.8360;

        % Correct the location
        bbox(1) = bbox(1) + bbox(3)*0.139;
        bbox(2) = bbox(2) + bbox(4)*0.138;            
...
%     imshow(image);hold on;plot(shape(visibilities,1), shape(visibilities,2), '.r');hold off;
...
%%
output_results = 'results/results_dev_clnf_janus.mat';

function Vanilla_CLNF_on_CS0_better_ROI()
...
for i=1:numel(images)
    ...
    new_width = bbox(3) * 0.6376;
    new_height = bbox(4) * 0.5117;
    new_tx = bbox(3) * 0.1517 + bbox(1);
    new_ty = bbox(4) * 0.4383 + bbox(2);
 
    bbox_det_new = [new_tx, new_ty, new_tx + new_width, new_ty + new_height];
    
    [shape,global_params,~,lhood,lmark_lhood,view_used] = Fitting_from_bb(image, [], bbox_det_new, pdm, patches, clmParams);
...
%%
output_results = 'results/results_janus_clnf_better_ROI.mat';

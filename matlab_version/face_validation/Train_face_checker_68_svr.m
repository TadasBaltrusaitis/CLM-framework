...
% Testing before writing them out

% Create the function
face_check_fun = @(img, shape, global_params) face_check_preloaded(img, shape, global_params, faceCheckers);

[ predictions, gts, rmse, corr_coeff ] = Test_face_checker(face_check_fun);

locationOut = [root_loc, './trained/face_check_general_68.txt'];
locationOutM = [root_loc, './trained/face_check_general_68.mat'];

save(locationOutM, 'faceCheckers', 'rmse', 'gts', 'predictions', 'corr_coeff');

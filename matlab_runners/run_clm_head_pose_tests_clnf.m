...
[bu_error_ccnf_ccnf_general, ~, ~, all_errors_bu_ccnf_general] = calcBUerror(resFolderBUccnf_general, buDir);
...
[biwi_error_ccnf_general, ~, ~, ~, all_errors_biwi_ccnf_general] = calcBiwiError([database_root res_folder_ccnf_general], [database_root biwi_dir]);
...
% Calculate the resulting errors
[ict_error_ccnf_general, ~, ~, ~, all_errors_ict_ccnf_general] = calcIctError([database_root res_folder_ict_ccnf_general], [database_root ict_dir]);
...
fprintf(f, 'Dataset and model,        roll,  pitch,  yaw,  mean,  median\n');

...
corr(all_hps(all_rels > 0.4, :), all_gts(all_rels > 0.4, :))

%%
pitch_err = mean(abs(all_hps(:,1) - all_gts(:,1)));
yaw_err = mean(abs(all_hps(:,2) - all_gts(:,2)));
roll_err = mean(abs(all_hps(:,3) - all_gts(:,3)));
